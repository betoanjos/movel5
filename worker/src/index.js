...
const segredo = (env) => {
  const s = env.SESSAO_SEGREDO;
  if (!s || s.length < 16) {
    throw new RespostaErro(500,
      'O segredo de sessão não foi configurado. Rode: npx wrangler secret put SESSAO_SEGREDO');
  }
  return s;
};

const chaveHmac = (env) =>
  crypto.subtle.importKey('raw', bytes(segredo(env)), { name: 'HMAC', hash: 'SHA-256' },
    false, ['sign', 'verify']);
